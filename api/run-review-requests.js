export default async function handler(req, res) {
  try {
    ...
    const bookingsRes = await fetch(
      `${supabaseUrl}/rest/v1/bookings?select=id,checkin,nights,guests,total,status,hotel_accounts(id,hotel_name,owner_email,whatsapp_number),customers(full_name,email,phone),rooms(name)&status=eq.confirmed`,
      {
        headers: {
          apikey: serviceKey,
    ...
    const checkedOutBookings = bookings.filter((booking) => {
      const checkinDate = new Date(booking.checkin);
      checkinDate.setDate(checkinDate.getDate() + Number(booking.nights || 1));
      ...
      const checkoutDate = `${yyyy}-${mm}-${dd}`;

      return checkoutDate === yesterdayDate;
    });
    ...
    for (const booking of checkedOutBookings) {
      ...
      const templatesRes = await fetch(
        `${supabaseUrl}/rest/v1/message_templates?select=*&hotel_id=eq.${hotel.id}&type=eq.checkout_thank_you&is_active=eq.true`,
        {
          headers: {
            apikey: serviceKey,
            Authorization: `Bearer ${serviceKey}`,
      ...
      for (const template of templates) {
        ...
        const subject = render(template.subject || "Thank you for staying with us");
    ...
    return res.status(200).json({
      success: true,
      checkout_date_checked: yesterdayDate,
      bookings_found: checkedOutBookings.length,
    ...
    return res.status(500).json({
      success: false,
      error: error.message || "Review automation failed",
    });
  }
}
